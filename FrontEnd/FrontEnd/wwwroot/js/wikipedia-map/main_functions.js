...
function expandNodeCallback(page, data) {
    ...
    for (var i = 0; i < subpages.length; i++) {
        addNodeAndEdge(subpages[i], page);
    }
    //for (var i = 0; i < subpages.length; i++) {
    ...
    //            x: nodeSpawn[0], y: nodeSpawn[1], isDefault: false, isAim: false, isMethod: false
    //        }); //Add node
    //    }

    //    if (!getEdgeConnecting(page, subpageID) && page != subpageID) { //Don't create duplicate edges in same direction
    //        newedges.push({
    //            from: page, to: subpageID, color: getEdgeColor(level),
    //            level: level, selectionWidth: 2, hoverWidth: 0, arrows: 'to'
    //        });
    //    }
    //}
